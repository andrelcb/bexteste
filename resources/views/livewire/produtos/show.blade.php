<div class="w-3/6 mx-auto">
    <x-card title="Lista de produtos">
        <x-input class="border-info-800" wire:model.defer="search" label="Pesquisar produto:"
            placeholder="Click na lupa para pesquisar 🧐 ">
            <x-slot name="append">
                <div class="absolute inset-y-0 right-0 flex items-center p-0.5">

                    <x-button class=" rounded-r-md h-full  " wire:click="render" icon="search" primary flat squared />
                </div>
            </x-slot>
        </x-input>
                    ...
                    @foreach ($produtos as $key => $row)
                        <tr class="hover:bg-gray-100 border-b border-gray-200 py-2 ">
                            ...
                            <td class="px-4 py-1 flex-row">
                                ...
                                <x-button wire:click="message({{ $row->id }},'{{ $row->name }}')" icon="x"
                                    negative />
